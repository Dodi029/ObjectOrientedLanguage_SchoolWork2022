...
public:
	...
	int widraw(int id, string password, int outmoney);				  // 계좌 출금

	int deposit(int id, int money);									  // 계좌 이체
	int getBalance() { return nBalance; }							  // ATM 총 잔고 읽어오기
	string getAccountName() { return strAccountName; }				  // 고객 이름 가져오기
};
